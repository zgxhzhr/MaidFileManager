...
import com.example.maid_file_manager.network.C2SPacket;
import com.example.maid_file_manager.network.ForgeNetwork;
import com.example.maid_file_manager.network.IMaidFileNetwork;
import com.example.maid_file_manager.network.S2CPacket;
...
public class MaidFileModForge {
    ...
    public MaidFileModForge() {
        ...
        com.example.maid_file_manager.config.MaidConfigManager.init(
                net.minecraftforge.fml.loading.FMLPaths.GAMEDIR.get());
    ...
    private void onRegisterCommands(net.minecraftforge.event.RegisterCommandsEvent event) {
        com.example.maid_file_manager.service.MaidServerCommands.register(event.getDispatcher());
        Constants.LOG.info("[maid_file_manager] Commands registered: /maidfile exportall");
    }

    /** 玩家登录：推送服务端配置同步（客户端据此显示设置界面并回发同意状态） */
    private void onPlayerLoggedIn(net.minecraftforge.event.entity.player.PlayerEvent.PlayerLoggedInEvent event) {
        if (event.getEntity() instanceof net.minecraft.server.level.ServerPlayer sp) {
            com.example.maid_file_manager.network.C2SPacket.broadcastServerConfig(sp.server);
        }
    }
    ...
    private void onPlayerLoggedOut(net.minecraftforge.event.entity.player.PlayerEvent.PlayerLoggedOutEvent event) {
        if (event.getEntity() instanceof net.minecraft.server.level.ServerPlayer sp) {
            com.example.maid_file_manager.config.MaidConfigManager.removeClientConsent(sp.getUUID());
        }
    }
